...
void CriaListaVaziaEncadeada(TListaEncadeada *listaE) //Função que cria uma lista encadeada
{
...
bool VerificaListaVaziaEncadeada(TListaEncadeada listaE) //Função para verificar se a lista encadeada esta vazia
{
    return (listaE.Primeiro == listaE.Ultimo);
}

void InserirEncadeada(TFuncionario fun, TListaEncadeada *listaE) //Função para insere algum item dentro da lista
{
...
void CriaListaVaziaSequencial(TListaSequencial *listaS) //Função para cria uma lista com arranjo
{
    listaS->primeiro = INICIO;
    listaS->ultimo = listaS->primeiro;
}

bool VerificaListaVaziaSequencial(TListaSequencial listaS) //Função para verificar se a lista com arranjo esta vazia
{
    if (listaS.ultimo == 0)
    {
        return true;
...
void InsereSequencial(TProjeto proj, TListaSequencial *listaS) //Função para inserir algum item na lista com arranjo
{
...
void CadastraFuncionario(TListaEncadeada *listaE) // Função para cadastrar os funcionários
{
    ...
    cin >> fun.endereco;
    while ((cout << "Infome o número de dependentes: ") && !(cin >> fun.dependentes)) //Verifica se o usúario digitou algo sem ser número
    {
...
void ConsultaFuncionario(TListaEncadeada *listaE) //Função que informa a codigo, nome, endereço, número de dependentes e os projetos do funcionário
{
    TChave cod;
    TApontador ptr;
    system("cls");
...
int PesquisaListaEncadeada(TChave cod, TListaEncadeada listaE, TApontador *ptr) //Função que realiza uma pesquisa na lista encadeada
{
...
void ImprimeProjetos(TListaSequencial listaS) //Função que iprime os projetos que foram cadastrados
{
...
int PesquisaListaSequencial(TListaSequencial *listaS, TChave cod) //Função que realiza uma pesquisa na lista com arranjo
{
    for (int i = 0; i < listaS->ultimo; i++)
    {
...
void ApagaProjeto(TApontador ptr, TListaSequencial *listaS, TChave cod, TProjeto *proj) //Função que apaga os projetos
{
    int indice;
